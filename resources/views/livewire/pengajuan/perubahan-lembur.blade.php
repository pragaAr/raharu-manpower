<div>
  ...
  @include('components.partials.header', ['title' => $title, 'permission' => 'pengajuan-lembur.create'])
  ...
  <div class="card">
    <div class="card-body border-bottom py-3">
      ...
      <div class="mb-3">
        <input type="text"
          id="search-pengajuan-perubahan-lembur"
          class="form-control"
          placeholder="Cari pengajuan perubahan lembur..."
        ...
        <table class="table table-vcenter table-bordered">
          <thead>
            <tr>
              ...
              <th class="fs-5 text-center">Jam Lama</th>
              <th class="fs-5 text-center">Jam Baru</th>
              <th class="fs-5 text-center">Status</th>
              <th class="fs-5">Alasan</th>
              @if($hasActions)
          ...
          <tbody>
            ...
            <tr wire:key="{{ $row->id }}">
              ...
              <td class="text-center">{{ $row->jam_mulai_lama?->format('H:i') ?? '-' }} - {{ $row->jam_selesai_lama?->format('H:i') ?? '-' }}</td>
              <td class="text-center">{{ $row->jam_mulai_baru?->format('H:i') ?? '-' }} - {{ $row->jam_selesai_baru?->format('H:i') ?? '-' }}</td>
              <td class="text-center text-uppercase">{{ $row->status }}</td>
              <td class="text-uppercase">{{ $row->alasan ?? '-' }}</td>
              @if($hasActions)
              <td class="text-center">
                <div class="btn-group" role="group" style="gap: 3px;">
                  @can('pengajuan-lembur.edit')
  ...
  @include('livewire.pengajuan.perubahan-lembur-modal')
