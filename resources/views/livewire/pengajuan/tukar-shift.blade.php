<div>
  ...
  @include('components.partials.header', ['title' => $title, 'permission' => 'pengajuan-tukar-shift.create'])
  ...
  <div class="card">
    <div class="card-body border-bottom py-3">
      ...
      <div class="mb-3">
        <input type="text"
          id="search-pengajuan-tukar-shift"
          class="form-control"
        ...
        <table class="table table-vcenter table-bordered">
          <thead>
            <tr>
              ...
              <th class="fs-5">Requester</th>
              <th class="fs-5">Target</th>
          ...
          <tbody>
            ...
            <tr wire:key="{{ $row->id }}">
              ...
              <td class="text-uppercase">
                <div class="fw-semibold">{{ $row->requester->nik ?? '-' }} - {{ $row->requester->nama ?? '-' }}</div>
                <small class="text-muted">
                  {{ strtoupper($row->requesterJadwal->shift->nama ?? $row->requesterJadwal->shift_nama ?? '-') }}
                  ({{ $row->requesterJadwal->jam_masuk?->format('H:i') ?? '-' }} - {{ $row->requesterJadwal->jam_pulang?->format('H:i') ?? '-' }})
                </small>
              </td>
              <td class="text-uppercase">
                <div class="fw-semibold">{{ $row->targetKaryawan->nik ?? '-' }} - {{ $row->targetKaryawan->nama ?? '-' }}</div>
                <small class="text-muted">
                  {{ strtoupper($row->targetJadwal->shift->nama ?? $row->targetJadwal->shift_nama ?? '-') }}
                  ({{ $row->targetJadwal->jam_masuk?->format('H:i') ?? '-' }} - {{ $row->targetJadwal->jam_pulang?->format('H:i') ?? '-' }})
                </small>
              </td>
              ...
              <td class="text-center">
                <div class="btn-group" role="group" style="gap: 3px;">
                  @can('pengajuan-tukar-shift.edit')
                  ...
                  @endcan
                  @can('pengajuan-tukar-shift.delete')
            ...
            @empty
            <tr>
              <td colspan="{{ $hasActions ? 7 : 6 }}" class="text-center text-muted">Belum ada data.</td>
            </tr>
            @endforelse
          </tbody>
        </table>
  ...
  @include('livewire.pengajuan.tukar-shift-modal')
